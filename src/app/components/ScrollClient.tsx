...
import { QrCode, Coffee, Scissors, Apple, Wallet, Sparkles } from "lucide-react";
...
export default function ScrollClient() {
  ...
  const digitalCards = [
    {
      name: "La Petite Torréfaction",
      type: "Coffee shop",
      stamps: 7,
      total: 10,
      reward: "Café offert",
      gradient: "linear-gradient(135deg, #0f2044 0%, #2c7be5 100%)",
      Icon: Coffee,
    },
    {
      name: "Salon Odyssée",
      ...
      stamps: 4,
      total: 8,
      reward: "-20 % sur votre prochaine coupe",
      gradient: "linear-gradient(135deg, #14285a 0%, #4a9eff 100%)",
      Icon: Scissors,
    },
    {
      name: "Boucherie Delmas",
      type: "Alimentaire",
      stamps: 9,
      total: 10,
      reward: "Sachet à offrir",
      gradient: "linear-gradient(135deg, #0b1220 0%, #1f63c7 100%)",
      Icon: Apple,
    },
  ];
        ...
        <div className="mx-auto max-w-7xl px-6 lg:px-10 w-full grid lg:grid-cols-2 gap-16 items-center">
          ...
          <div>
            ...
            <p className="mt-6 text-[#47526a] text-[17px] leading-relaxed max-w-lg">
              ...
              fidélité, instantanément réunies, organisées, et prêtes à l&apos;emploi.
            </p>
          ...
          <div className="relative h-[620px] flex items-center justify-center" data-testid="client-cards-stage">
            {/* Paper card 1 */}
            <motion.div
              style={{ y: paper1Y, rotate: paper1R, opacity: paper1O }}
              className="absolute w-[300px] h-[170px] rounded-xl"
            >
              <div
                className="w-full h-full rounded-xl p-5 relative overflow-hidden"
                style={{
                  background: "linear-gradient(135deg, #f7f2e6 0%, #e8dfc9 100%)",
                  boxShadow: "0 12px 24px -12px rgba(60,40,15,.3), inset 0 0 0 1px rgba(0,0,0,.04)",
                }}
              >
                <div
                  className="absolute inset-0 opacity-40"
                  style={{ backgroundImage: "repeating-linear-gradient(45deg, rgba(0,0,0,.02) 0 2px, transparent 2px 6px)" }}
                />
            ...
            {/* Paper card 2 */}
            <motion.div
              style={{ y: paper2Y, rotate: paper2R, opacity: paper2O }}
              className="absolute w-[280px] h-[160px] rounded-xl"
            >
              <div
                className="w-full h-full rounded-xl p-5"
                style={{
                  background: "linear-gradient(135deg, #fbfaf4 0%, #ede7d2 100%)",
                  boxShadow: "0 10px 22px -10px rgba(60,40,15,.28), inset 0 0 0 1px rgba(0,0,0,.04)",
                }}
              >
            ...
            {/* Paper card 3 */}
            <motion.div
              style={{ y: paper3Y, rotate: paper3R, opacity: paper3O }}
              className="absolute w-[260px] h-[150px] rounded-xl"
            >
              <div
                className="w-full h-full rounded-xl p-4"
                style={{
                  background: "linear-gradient(135deg, #f4ecd8 0%, #d8c897 100%)",
                  boxShadow: "0 8px 20px -10px rgba(60,40,15,.25), inset 0 0 0 1px rgba(0,0,0,.04)",
                }}
              >
              ...
              return (
                <motion.div
                  ...
                  style={{
                    opacity: o,
                    y: yv,
                    left: `calc(50% + ${offset.left}px - 165px)`,
                    top: `calc(50% + ${offset.top}px - 105px)`,
                    rotate: (i - 1) * 5,
                    zIndex: 10 - i,
                  }}
                ...
                >
                  <div
                    ...
                    style={{
                      ...
                      boxShadow: "0 24px 60px -20px rgba(15,32,68,.4), inset 0 1px 0 rgba(255,255,255,.15)",
                    }}
                  >
                    ...
                    <div className="flex items-start justify-between">
                      <div>
                        ...
                        <div className="text-white text-[18px] font-bold mt-0.5" style={{ fontFamily: "Manrope" }}>
                          {card.name}
                        </div>
                      </div>
                    ...
                    <div className="mt-5 flex flex-wrap gap-1.5">
                      {Array.from({ length: card.total }).map((_, idx) => (
                        <div
                          key={idx}
                          className={`w-5 h-5 rounded-md ${idx < card.stamps ? "bg-white" : "border border-white/30"}`}
                    ...
                    <div className="mt-3 h-1 rounded-full bg-white/12 overflow-hidden">
                      <div
                        ...
                        style={{ width: `${(card.stamps / card.total) * 100}%` }}
                      />
                    </div>
                  </div>
                </motion.div>
