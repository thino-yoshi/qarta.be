...
import { Bell, Star, Gift, Clock, TrendingUp, Stamp } from "lucide-react";
...
const notifs: Notif[] = [
  {
    ...
    body: "Marie a laissé 5 étoiles · +1 récompense créditée.",
    tint: "#fef3c7",
    iconColor: "#f59e0b",
    side: "right",
    delayStart: 0.25,
  ...
  {
    icon: Bell,
    title: "Rappel de visite envoyé",
    body: "12 clients relancés sans effort.",
    tint: "#eaf2fd",
    iconColor: "#2c7be5",
    side: "left",
    delayStart: 0.35,
  ...
  {
    icon: Gift,
    title: "3 récompenses à distribuer",
    body: "Vos clients réguliers en attente.",
    tint: "#dcfce7",
    iconColor: "#27ae60",
    side: "right",
    delayStart: 0.45,
  ...
  {
    icon: Clock,
    title: "Invendus à écouler",
    body: "Push automatique aux habitués → 15% utilisé.",
    tint: "#fee2e2",
    iconColor: "#e24b4a",
    side: "left",
    delayStart: 0.55,
...
function NotifCard({
  ...
  const x = useTransform(
    scrollYProgress,
    [start, end],
    n.side === "right" ? [-30, 200 + i * 10] : [30, -220 - i * 10]
  );
  ...
  return (
    ...
    >
      <div
        className="w-[280px] p-3.5 rounded-2xl flex items-start gap-3"
        style={{
          background: "rgba(255,255,255,0.95)",
          backdropFilter: "blur(16px)",
          boxShadow: "0 20px 50px -20px rgba(11,18,32,0.5), 0 0 0 1px rgba(255,255,255,0.6)",
        }}
      >
        <div
    ...
    <div className="w-full h-full flex flex-col text-white" style={{ background: "#0b1220" }}>
      <div className="px-5 pt-12 pb-4 relative">
        ...
        <div className="relative flex items-center justify-between">
          ...
          <div className="w-8 h-8 rounded-lg bg-[#0f2044] border border-[#4a9eff]/30 flex items-center justify-center text-[12px] font-bold text-[#4a9eff]">
            Q
          </div>
        </div>
      </div>
        ...
        <div className="mt-3 flex items-end gap-1.5 h-16">
          {[28, 40, 52, 34, 62, 72, 58].map((h, i) => (
            <div
              ...
              style={{
                height: `${h}%`,
                background: "linear-gradient(180deg, #4a9eff, #2c7be5)",
                opacity: 0.4 + i * 0.08,
              }}
            />
          ))}
...
export default function ScrollMerchant() {
  ...
  return (
    ...
    >
      ...
      />
      <div
        className="absolute bottom-0 left-[-10%] w-[45vw] h-[45vw] rounded-full opacity-20 blur-3xl"
        ...
        <div className="relative mx-auto max-w-7xl px-6 lg:px-10 w-full grid lg:grid-cols-12 gap-12 items-center">
          <div className="lg:col-span-5">
            ...
            <div className="mt-8 grid grid-cols-2 gap-3 max-w-md">
              {[
                { icon: Stamp, label: "Tampons & récompenses" },
                { icon: Bell, label: "Notifications automatiques" },
                { icon: TrendingUp, label: "Statistiques en temps réel" },
                { icon: Star, label: "Avis Google intégrés" },
              ].map(({ icon: Icon, label }) => (
                <div
                  key={label}
                  className="flex items-center gap-2.5 p-3 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-sm"
                >
